...
private void listarTabela(){
    ...
    private void preencherCampos(MouseEvent e){
        ...
        id = (int) view.getTblClientes().getModel().getValueAt(target.getSelectedRow(),0);
        view.getTxtNome().setText(view.getTblClientes().getModel().getValueAt(target.getSelectedRow(), 1).toString());
        view.getTxtContacto().setText(view.getTblClientes().getModel().getValueAt(target.getSelectedRow(), 2).toString());
        view.getTxtEndereco().setText(view.getTblClientes().getModel().getValueAt(target.getSelectedRow(), 3).toString());
    }
    ...
    private void actualizarCliente(){
        ...
        try {
            if(validarDados()){
                if(carregarDados()){
                    Cliente clienteUp = new Cliente(id, nome,contacto,endereco);
                    c.actualizar(clienteUp);
    ...
    private void apagarCliente(){
        try {
            ...
            JOptionPane.showMessageDialog(null,"Erro ao apagar o cliente","Erro de cadastroo",JOptionPane.ERROR_MESSAGE);
            System.out.println("Erro ao apagar o cliente "+e);
        }finally{
            listarTabela();
        }
    }
